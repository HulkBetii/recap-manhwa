...
async def main():
    ...
    print("=" * 75)
    print("  START WORKFLOW: The World After the Fall - Tập 1-2-3 (Tiếng Việt)")
    ...
    # Pre-seed episode_1 cache from existing run if available
    src_ep1 = os.path.join("downloads", "the_world_after_the_fall_1_1_vi_be5d3841", "episode_1")
    dest_dir = os.path.join("downloads", "the_world_after_the_fall_1_3_vi_be5d3841")
    dest_ep1 = os.path.join(dest_dir, "episode_1")
    if os.path.isdir(src_ep1) and not os.path.exists(dest_ep1):
        os.makedirs(dest_dir, exist_ok=True)
        print(f"[INFO] Pre-seeding episode_1 cache from {src_ep1} to {dest_ep1}...")
        shutil.copytree(src_ep1, dest_ep1)
        src_mem = os.path.join("downloads", "the_world_after_the_fall_1_1_vi_be5d3841", "story_memory.json")
        dest_mem = os.path.join(dest_dir, "story_memory.json")
        if os.path.exists(src_mem) and not os.path.exists(dest_mem):
            shutil.copy2(src_mem, dest_mem)
        print("[SUCCESS] Pre-seeded episode_1 and story_memory.json successfully.")

    task_id = f"the-world-after-the-fall-ep1-3-vi-{int(time.time())}"
    task = WorkflowTask(
    ...
    print("\n=======================================================")
    print("  CHÚC MỪNG: TẬP 1-2-3 TIẾNG VIỆT ĐÃ HOÀN THÀNH XUẤT SẮC!")

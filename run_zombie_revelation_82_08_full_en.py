...
class ConsoleContext:
    ...
    async def start_episode(self, ep: int):
        ...
        await self.log(f"Bắt đầu xử lý tập {ep}/{self.task.to_episode}...", "info")

    async def complete_episode(self, ep: int):
        await self.log(f"Hoàn thành tập {ep}/{self.task.to_episode}.", "success")
...
async def main():
    ...
    print("=" * 80)
    print("  START RECAP WORKFLOW: Zombie Revelation: 82-08 (Episodes 1–120 — Full Story)")
    ...
    task_id = f"zombie-revelation-82-08-full-1-120-en-{int(time.time())}"
    task = WorkflowTask(
    ...
    print("\n" + "=" * 80)
    print("  CHÚC MỪNG: TRỌN BỘ 120 TẬP ZOMBIE REVELATION TIẾNG ANH ĐÃ HOÀN THÀNH XUẤT SẮC!")

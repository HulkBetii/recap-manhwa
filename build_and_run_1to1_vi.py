...
def sync_1to1_recap(en_dir, vi_dir):
    ...
    for ep in [1, 2]:
        ...
        if len(existing_vi_scenes) == len(en_scenes):
            print(f"[1:1 SYNC] Episode {ep}: Reusing {len(existing_vi_scenes)} existing 1:1 Vietnamese translated scenes.", flush=True)
            merged_scenes = []
            ...
            print(f"[1:1 SYNC] Translating Episode {ep} ({len(en_scenes)} scenes) via MyMemory...", flush=True)
            merged_scenes = []
            for i, en_s in enumerate(en_scenes):
        ...
        print(f"[1:1 SYNC] Episode {ep}: Synchronized {len(merged_scenes)} scenes 1:1 with Vietnamese narration.", flush=True)
...
async def main():
    ...
    en_dir = os.path.join(project_dir, "downloads", "surviving_the_apocalypse_1_2_en_947dedf7")
    vi_dir = os.path.join(project_dir, "downloads", "surviving_the_apocalypse_1_2_vi_947dedf7")
    
    sync_1to1_recap(en_dir, vi_dir)

    # Clean VI output directory
    vi_output_dir = os.path.join(vi_dir, "output")
    if os.path.exists(vi_output_dir):
        shutil.rmtree(vi_output_dir)
    ...
    task.artifacts["download_folder_name"] = os.path.basename(vi_dir)
